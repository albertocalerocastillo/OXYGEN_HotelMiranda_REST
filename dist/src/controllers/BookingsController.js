...
const getBookingsController = async (req, res) => {
    try {
        const bookings = await BookingService_1.bookingService.getBookings();
        res.json(bookings);
    }
    catch (error) {
...
const getBookingController = async (req, res) => {
    try {
        const booking = await BookingService_1.bookingService.getBooking(req.params.id);
        if (!booking) {
...
const createBookingController = async (req, res) => {
    try {
        const { error } = BookingValidator_1.validateBookingCreate.validate(req.body);
        if (error) {
            return res.status(400).json({ message: error.details[0].message });
        }
        const bookingData = req.body;
        const room = await RoomService_1.roomService.getRoom(bookingData.room.id);
        if (!room) {
        ...
        await BookingService_1.bookingService.createBooking(booking);
...
const updateBookingController = async (req, res) => {
    try {
        ...
        const room = await RoomService_1.roomService.getRoom(updatedBookingData.room.id);
        if (!room) {
            return res.status(400).json({ message: 'La habitación especificada no existe' });
        }
        const updatedBooking = { ...updatedBookingData, room: room };
        await BookingService_1.bookingService.updateBooking(id, updatedBooking);
